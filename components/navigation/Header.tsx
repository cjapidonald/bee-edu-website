...
export default function Header({ lang, dict }: HeaderProps) {
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://app.kiwibee.io";
  ...
  const FeatureLinkItem = ({ link }: { link: { name: string; path: string; icon: LucideIcon; badge?: string } }) => (
    ...
    >
      ...
      {link.badge && (
        <span className="text-[8px] font-medium bg-gradient-to-r from-[#16a34a] to-[#15803d] text-white px-1 py-0.5 rounded flex-shrink-0">
          {link.badge}
        </span>
      )}
    </Link>
