import { TrendingUp, Target, BarChart3, Eye, MessageSquare, Award } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.kpi;
  const title = t.seoTitle || `${t.title} ${t.highlight} | KiwiBee`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/teacher-kpi", title, description });
}

export default async function TeacherKPIPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
