import { BarChart3, Target, Palette, TrendingUp, FileText, Users, Brain } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.gradebook;
  const title = t.seoTitle || `${t.title} ${t.highlight} | KiwiBee`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/gradebook", title, description });
}

export default async function GradebookPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
  ...
  return (
    <FeaturePageTemplate
      ...
      lang={lang}
      mockup={<GradebookMockup lang={lang} />}
    />
  );
}
