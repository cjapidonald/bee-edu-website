import { CalendarDays, Building, Laptop, AlertTriangle, CheckCircle, BarChart3, Bell } from "lucide-react";
...
import { ResourceBookingMockup } from "@/components/beaver-mockups";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.resourceBooking;
  const title = t.seoTitle || `${t.title} ${t.highlight} | Elementals`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/resource-booking", title, description });
}

export default async function ResourceBookingPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
  ...
  return (
    <FeaturePageTemplate
      ...
      lang={lang}
      mockup={<ResourceBookingMockup lang={lang} />}
    />
  );
}
