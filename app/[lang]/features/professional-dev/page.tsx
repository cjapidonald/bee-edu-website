import { Award, BookOpen, Calendar, Target, Users, TrendingUp } from "lucide-react";
...
import { ProfessionalDevMockup } from "@/components/beaver-mockups";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.professionalDevelopment;
  const title = t.seoTitle || `${t.title} ${t.highlight} | Elementals`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/professional-dev", title, description });
}

export default async function ProfessionalDevPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
  ...
  return (
    <FeaturePageTemplate
      ...
      lang={lang}
      mockup={<ProfessionalDevMockup lang={lang} />}
    />
  );
}
