import { ShieldCheck, FileText, CheckCircle, Calendar, BarChart3, ClipboardList } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.qa;
  const title = t.seoTitle || `${t.title} ${t.highlight} | Bee Education AI`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/qa-accreditation", title, description });
}

export default async function QAAccreditationPage({ params }: { params: Promise<{ lang: string }> }) {
  const lang = (await params).lang === "zh-HK" ? "zh-HK" : "en";
  const copy = marketingCopy[lang] ?? marketingCopy.en;
  const t = copy.featurePages.qa;
