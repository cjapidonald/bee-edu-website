import { CalendarClock, Users, ArrowRightLeft, CheckCircle, Shield, History } from "lucide-react";
...
import { YearMigrationMockup } from "@/components/beaver-mockups";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.yearMigration;
  const title = t.seoTitle || `${t.title} ${t.highlight} | Elementals`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/year-migration", title, description });
}

export default async function YearMigrationPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
  ...
  return (
    <FeaturePageTemplate
      ...
      lang={lang}
      mockup={<YearMigrationMockup lang={lang} />}
    />
  );
}
