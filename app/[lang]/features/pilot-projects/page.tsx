import { FlaskConical, Target, BarChart3, Users, Clock, CheckCircle } from "lucide-react";
...
import { AdminDashboardMockup } from "@/components/mockups";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = copy.featurePages.pilotProjects;
  const title = t.seoTitle || `${t.title} ${t.highlight} | KiwiBee`;
  const description = t.seoDescription || t.description || "";
  return buildPageMetadata({ lang, path: "/features/pilot-projects", title, description });
}

export default async function PilotProjectsPage({ params }: { params: Promise<{ lang: string }> }) {
  const rawLang = (await params).lang;
